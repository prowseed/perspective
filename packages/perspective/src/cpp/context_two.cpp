...
t_ctx2::init()
{
    ...
    m_rtraversal = std::make_shared<t_traversal>(
        rtree(), m_config.handle_nan_sort());

    m_ctraversal = std::make_shared<t_traversal>(
        ctree(), m_config.handle_nan_sort());
...
t_index
t_ctx2::get_row_count(t_header header) const
{
    switch (header)
    {
        case HEADER_ROW:
...
t_vdnvec
t_ctx2::get_view_nodes(t_tvidx start_row, t_tvidx end_row) const
{
    return m_rtraversal->get_view_nodes(start_row, end_row);
}
...
t_ctx2::get_ctraversal_indices() const
{
    switch (m_config.get_totals())
    {
        ...
        case TOTALS_HIDDEN:
        {
            ...
            rval[0] = 0;
            for (t_uindex idx = 1, loop_end = rval.size();
                 idx < loop_end;
                 ++idx)
            {
                 ...
                 t_tvidx end_col) const
{
    ...
    {
        for (t_index cidx = 1; cidx < end_col; ++cidx)
        {
    ...
    for (t_index ridx = ext.m_srow; ridx < ext.m_erow; ++ridx)
    {
        ...
        for (t_index cidx = 1; cidx < end_col; ++cidx)
        {
            t_index insert_idx = (ridx - ext.m_srow) * stride + cidx;
            const t_cellinfo& cinfo = cells_info[cinfoidx];
               ...
               const t_table& existed)
{
    ...
    for (t_uindex tree_idx = 0, loop_end = m_trees.size();
         tree_idx < loop_end;
         ++tree_idx)
    {
        if (is_rtree_idx(tree_idx))
        {
            notify_sparse_tree(rtree(),
                               m_rtraversal,
                               true,
    ...
    switch (m_config.get_totals())
    {
        ...
        default:
        {
            PSP_COMPLAIN_AND_ABORT(
                "Unknown totals type encountered.");
            return -1;
        }
    }
}
...
t_ctx2::resolve_cells(const t_uidxpvec& cells) const
{
    ...
    for (t_index cidx = 0, loop_end = c_tvindices.size();
         cidx < loop_end;
         ++cidx)
    {
         ...
         ++idx)
    {
        ...
        t_uindex translated_cidx =
            calc_translated_colidx(n_aggs, cell.second);
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    rval[idx].m_idx = m_trees[tree_idx]->resolve_path(
                        path_ptidx, c_path);
                }
            }
        }
...
t_uindex
t_ctx2::get_max_levels(t_header header) const
{
    return get_num_levels(header);
}

t_uindex
t_ctx2::get_num_levels(t_header header) const
{
    switch (header)
    {
        case HEADER_ROW:
            return m_config.get_num_rpivots() + 1;
        case HEADER_COLUMN:
            return m_config.get_num_cpivots() + 1;
        default:
            PSP_COMPLAIN_AND_ABORT("Unknown header type");
            return INVALID_INDEX;
    }
}
...
t_ctx2::get_num_view_columns() const
{
    switch (m_config.get_totals())
    {
        ...
        {
            return m_ctraversal->size() *
                       m_config.get_num_aggregates() +
                   1;
        }
        break;
        case TOTALS_HIDDEN:
        {
            t_index nitems = (m_ctraversal->size() - 1) *
                             m_config.get_num_aggregates();
            return nitems + 1;
        }
        break;
...
t_ctx2::translate_column_index(t_tvidx idx) const
{
    ...
    switch (m_config.get_totals())
    {
        ...
        case TOTALS_AFTER:
        {
            ...
            m_ctraversal->post_order(0, col_order);
            rval =
                col_order[(idx - 1) / m_config.get_num_aggregates()];
        }
        break;
        case TOTALS_HIDDEN:
        {
...
t_trav_csptr
t_ctx2::get_traversal(t_header header) const
{
    switch (header)
    {
        case HEADER_ROW:
...
t_tscalar
t_ctx2::get_tree_value(t_header header, t_ptidx nidx) const
{
    switch (header)
    {
        case HEADER_ROW:
            return rtree()->get_value(nidx);
        case HEADER_COLUMN:
            return ctree()->get_value(nidx);
        default:
        {
            PSP_COMPLAIN_AND_ABORT("Invalid header");
        }
...
t_aggspec
t_ctx2::get_aggregate(t_uindex idx) const
{
...
t_pathvec
t_ctx2::get_expansion_state(t_header header) const
{
    switch (header)
    {
        case HEADER_ROW:
        {
            return ctx_get_expansion_state(rtree(), m_rtraversal);
        }
        break;
        case HEADER_COLUMN:
        {
            return ctx_get_expansion_state(ctree(), m_ctraversal);
        }
        break;
        default:
        {
...
void
t_ctx2::set_expansion_state(t_header header, const t_pathvec& paths)
{
    switch (header)
    {
        case HEADER_ROW:
        {
            ctx_set_expansion_state(
                *this, HEADER_ROW, rtree(), m_rtraversal, paths);
        }
        break;
        case HEADER_COLUMN:
        {
            ctx_set_expansion_state(
                *this, HEADER_COLUMN, ctree(), m_ctraversal, paths);
        }
        break;
        default:
        {
    ...
    switch (header)
    {
        case HEADER_ROW:
        {
            if (m_config.get_num_rpivots() == 0)
                return;
            expand_depth = std::min<t_depth>(
                m_config.get_num_rpivots() - 1, depth);
        ...
        case HEADER_COLUMN:
        {
            if (m_config.get_num_cpivots() == 0)
                return;
            expand_depth = std::min<t_depth>(
                m_config.get_num_cpivots() - 1, depth);
            m_ctraversal->expand_to_depth(m_column_sortby,
                                          expand_depth);
        }
        break;
        default:
...
void
t_ctx2::expand_path(t_header header, const t_tscalvec& path)
{
    switch (header)
    {
        case HEADER_ROW:
        {
            ctx_expand_path(
                *this, HEADER_ROW, rtree(), m_rtraversal, path);
        }
        break;
        case HEADER_COLUMN:
        {
            ctx_expand_path(
                *this, HEADER_COLUMN, ctree(), m_ctraversal, path);
        }
        break;
        default:
        {
...
t_ctx2::get_pkeys(const t_uidxpvec& cells) const
{
    ...
    for (t_index idx = 0, loop_end = tree_info.size(); idx < loop_end;
         ++idx)
    {
        ...
        {
            auto node_pkeys =
                m_trees[cinfo.m_treenum]->get_pkeys(cinfo.m_idx);
            std::copy(node_pkeys.begin(),
                      node_pkeys.end(),
...
t_ctx2::get_step_delta(t_tvidx bidx, t_tvidx eidx)
{
    ...
    for (const auto& c : cells_info)
    {
        ...
        auto iterators =
            deltas->get<by_tc_nidx_aggidx>().equal_range(c.m_idx);

        for (auto iter = iterators.first; iter != iterators.second;
             ++iter)
        {
            updvec.push_back(t_cellupd(c.m_ridx,
                                       c.m_cidx,
                                       iter->m_old_value,
                                       iter->m_new_value));
        }
...
t_ctx2::reset()
{
         ...
         ++treeidx)
    {
        ...
        m_trees[treeidx] = std::make_shared<t_stree>(
            pivots, m_config.get_aggregates(), m_schema, m_config);
        m_trees[treeidx]->init();
        m_trees[treeidx]->set_deltas_enabled(
            get_feature_state(CTX_FEAT_DELTA));
    }
...
t_ctx2::has_deltas() const
{
    ...
    for (t_uindex idx = 0, loop_end = m_trees.size(); idx < loop_end;
         ++idx)
    {
...
t_index
t_ctx2::get_row_idx(const t_tscalvec& path) const
{
    auto nidx = rtree()->resolve_path(0, path);
    if (nidx == INVALID_INDEX)
    {
        return nidx;
    }

    return m_rtraversal->get_traversal_index(nidx);
}
...
t_tvnode
t_ctx2::get_trav_node(t_header header, t_tvidx idx) const
{
    switch (header)
    {
        case HEADER_ROW:
            return m_rtraversal->get_node(idx);
        case HEADER_COLUMN:
            return m_ctraversal->get_node(idx);
        default:
        {
            PSP_COMPLAIN_AND_ABORT("Invalid header");
        }
...
t_index
t_ctx2::get_trav_num_tree_leaves(t_header header, t_tvidx idx) const
{
    switch (header)
    {
        case HEADER_ROW:
            return m_rtraversal->get_num_tree_leaves(idx);
        case HEADER_COLUMN:
            return m_ctraversal->get_num_tree_leaves(idx);
        default:
        {
            PSP_COMPLAIN_AND_ABORT("Invalid header");
        }
